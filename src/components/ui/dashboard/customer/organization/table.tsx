...
import { StatsCustomerOrganization, TTable } from '@/types';

type Props = {} & Omit<
  TTable<StatsCustomerOrganization>,
...
const columns = [
  ...
  { key: 'tenLhd', label: 'Loại hợp đồng' },
  { key: 'soTien', label: 'Số tiền' },
];

const TableDashboardCustomerOrganization: React.FC<Props> = ({
  isLoading,
  data,
  filter,
  setFilter,
  ...
  return (
    <>
      ...
      >
        ...
        >
          {(item) => (
            <TableRow key={`${item.maKh}-${item.tenLhd}`}>
              {(columnKey) => (
...
export default TableDashboardCustomerOrganization;
