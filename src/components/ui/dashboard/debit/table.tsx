...
import { StatsDebit, TTable } from '@/types';

type Props = {} & Omit<TTable<StatsDebit>, 'selectedKeys' | 'setSelectedKeys'>;

const columns = [
  { key: 'maTk', label: 'Mã tài khoản' },
  { key: 'thoiGian', label: 'Thời gian' },
  { key: 'tenKh', label: 'Tên khách hàng' },
  { key: 'soTien', label: 'Số tiền nợ' },
];

const TableDashboardDebit: React.FC<Props> = ({
  isLoading,
  data,
  filter,
  setFilter,
  ...
  return (
    <>
      ...
      >
        ...
        >
          {(item) => (
            <TableRow key={`${item.maTk}-${item.thoiGian}`}>
              {(columnKey) => (
...
export default TableDashboardDebit;
