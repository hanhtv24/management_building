...
import { depositMoney } from '@/lib/action/account';
...
const DepositModal: React.FC<Props> = ({
  ...
  const {
    ...
    defaultValues: {
      maTkDich: selected.maTk,
      maNvxl: selected.nhanVien.maNv,
      soTien: 1,
      noiDung: `Gửi tiền vào tài khoản ${selected.maTk}`,
    },
  ...
  const onSubmit: SubmitHandler<z.infer<typeof ValidateSchema>> = async (
    ...
    try {
      await depositMoney({ ...rest, ...numericFields });
    } catch (e) {
      addToast({
  ...
  return (
    ...
    >
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader className="flex flex-col gap-1">Gửi tiền</ModalHeader>
            <Form
