...
import { withdrawMoney } from '@/lib/action/account';
...
const WithdrawModal: React.FC<Props> = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      soTien: 1,
      noiDung: `Rút tiền từ tài khoản ${selected.maTk}`,
    },
  ...
  const onSubmit: SubmitHandler<z.infer<typeof ValidateSchema>> = async (
    ...
    try {
      await withdrawMoney({ ...rest, ...numericFields });
    } catch (e) {
  ...
  return (
    ...
    >
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader className="flex flex-col gap-1">Rút tiền</ModalHeader>
            <Form
