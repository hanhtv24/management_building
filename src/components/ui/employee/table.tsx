...
import { Employee, TTable } from '@/types';

type Props = {} & TTable<Employee>;
...
const TableEmployee: React.FC<Props> = ({
  isLoading,
  data,
  filter,
  setFilter,
  ...
  const renderCell = useCallback(
    (data: Props['data']['data'][number], columnKey: React.Key) => {
      ...
      switch (columnKey) {
        ...
        case 'chucVu':
          return data.chucVu?.tenCv || '-'.repeat(6);
        case 'chiNhanh':
          return data.chiNhanh?.tenCn || '-'.repeat(6);
        default:
          if (
